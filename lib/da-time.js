...
export default class DATime {
  ...
  constructor(data) {
    let input = data instanceof DATime ? data.timestamp : data;
    this.input = input ?? 1;
  ...
  formatDate({ separator = '/' } = {}) {
    return `${DATime.stringN(this.day)}${separator}${DATime.stringN(this.month)}${separator}${DATime.convertYearToString(this.year)}`;
  }
